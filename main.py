...
def ending_insert(linestart, endstatement):
    ...
    try:
        while code_indent(
                current_line
        ) > spaces or "else" in python_processing[current_line]:
            current_line += 1
            if len(
                    python_processing
            ) == current_line:  #Handles situations where the indent runs to end of file
                break
    except:
...
i = 0
while i < len(
        python_processing):  # Need to use while, because list gets bigger!

    if "else:" in python_processing[i]:
        python_processing[i] = python_processing[i].replace("else:", 'ELSE')

    if "elif" in python_processing[i]:
        python_processing[i] = python_processing[
            i] + "  //You will need to change this to CASE OF"
...
while i < len(
    ...
    if "for " in python_processing[i]:
        ...
        if "range" in python_processing[i]:
            ...
            python_processing[
                i] = python_processing[i] + "  //Pseudocode can't handle this"
...
for i in range(len(python_processing)):
    if "break" in python_processing[i]:
        python_processing[i] = python_processing[
            i] + "  //This might be better as a repeat loop"
